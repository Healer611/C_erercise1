...
//	int* ptr2 = (int*)(*(aa + 1));
//	/*
//	在给定的代码中，(int*)(*(aa + 1))表示将第二个一维数组的第一个元素的地址转换为int* 类型的指针。
//	(int*)是一种类型转换操作符，它将括号内的表达式转换为指定类型的指针。在这里，(int*)将* (aa + 1)的结果转换为int* 类型的指针。
//		在没有类型转换的情况下，* (aa + 1)的结果是一个int类型的值，即第二个一维数组的第一个元素的值，即6。
//		通过进行类型转换(int*)(*(aa + 1))，我们将这个int类型的值的地址转换为了int* 类型的指针。这样，指针ptr2就指向了数组aa中第二个一维数组的第一个元素。
//		这种类型转换的目的可能是为了在后续的代码中以指针的形式访问和操作这个值，而不仅仅是将其视为一个整数值。
//	*/
